...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, indexStart) + str.slice(indexEnd);
}
...
function countVowels(str) {
  let count = 0;
  for (let i = 0; i < str.length; i += 1) {
    if ('aeiouy'.includes(str[i].toLowerCase())) {
      count += 1;
    }
  }
  return count;
...
function isPalindrome(str) {
  const remove = str.replaceAll('!', '');
  const remove1 = remove.replaceAll('?', '');
  const str1 = remove1.split(',').join('');
  const str2 = str1.split(' ').join('');

  const strReverse = str2.split('').reverse().join('');
  if (str2.toLowerCase() === strReverse.toLowerCase()) {
    return true;
  }
  return false;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentenceSplit.length; i += 1) {
    if (sentenceSplit[i].length > wordResult.length) {
      wordResult = sentenceSplit[i];
    }
  }
  return wordResult;
}
